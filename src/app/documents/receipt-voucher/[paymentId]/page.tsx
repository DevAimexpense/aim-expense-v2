...
// สำหรับรายจ่ายที่แนบสำเนาบัตรประชาชน + ไม่มีการหัก ณ ที่จ่าย
// ===========================================
...
import { ReceiptVoucherDocument } from "./document";
...
export default async function ReceiptVoucherPage({
  ...
  console.log(`[receipt-voucher/page] render paymentId=${paymentId}`);
  const sheets = await getSheetsService(orgCtx.orgId);
  const payment = await getPaymentWithRetry(sheets, paymentId);
  if (!payment) {
    console.error(`[receipt-voucher/page] payment ${paymentId} NOT FOUND`);
    return (
